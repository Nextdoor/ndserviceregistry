...
class nd_service_registry(object):
    ...
    def lock(self, path, name=None, simultaneous=1):
        """Retreives a lock from the supplied path.
        ...
        args:
            ...
            simultaneous: Optional integer indiciating how many clients can
    ...
    def acquire_lock(self, path, name=None, simultaneous=1, wait=0):
        ...
        Retreives a Lock object for a particular path and 'acquires' the lock.
        It returns True if the lock is sucessfull. If the lock path is busy,
        waits until the the lock can be acquired.

        args:
            path: String representing the lock path
            name: Optional string representing the name of the client
            simultaneous: Optional integer indiciating how many clients can lock
    ...
    def release_lock(self, path):
        ...
        Retreives a Lock object for a particular path and releases the lock on that
        path.
...
class KazooServiceRegistry(nd_service_registry):
    ...
    def _get_lock(self, path, name=None, simultaneous=1):
        """Retreives a lock semaphore-style object from the supplied path.
