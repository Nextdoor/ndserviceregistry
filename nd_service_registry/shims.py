...
class ZookeeperClient(KazooClient):
    ...
    def rate_limiter(func):
        ...
        def _rate_limited_function(self, *args, **kwargs):
            ...
            # Is rate limiting even enabled? Do the variables exist?
            if not (hasattr(self, 'num_calls_to_average_over') and
                    hasattr(self, 'target_avg_between_calls')):
                self.set_rate_limiter()

            # Make sure that if the values are None or False, reset them
            if not (isinstance(self.num_calls_to_average_over, int) and
                    isinstance(self.target_avg_between_calls, int)):
                self.set_rate_limiter()
